...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private void initGame() {
        int px = Math.max(0, getWidth() / 2 - Player.DEFAULT_WIDTH / 2);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                bullets.add(new Bullet(player.x + player.width / 2 - Bullet.DEFAULT_WIDTH / 2, player.y));
    ...
    public void update() {
        ...
        if (now - lastAsteroidTime > 800) {
            int maxX = Math.max(1, getWidth() - Asteroid.DEFAULT_SIZE);
            int ax = random.nextInt(maxX);
            asteroids.add(new Asteroid(ax, -Asteroid.DEFAULT_SIZE));
            lastAsteroidTime = now;
        }

        if (bullets != null) for (Bullet b : bullets) b.update();
        if (asteroids != null) for (Asteroid a : asteroids) a.update();

        if (bullets != null) {
            Iterator<Bullet> bi = bullets.iterator();
            while (bi.hasNext()) {
                Bullet b = bi.next();
                if (b.y + b.height < 0) bi.remove();
            }
        }
        ...
        if (asteroids != null) {
            ...
            while (ai.hasNext()) {
                Asteroid a = ai.next();
                if (a.y - a.size > getHeight()) ai.remove();
            }
        }
        ...
        if (bullets != null && asteroids != null) {
            for (Iterator<Bullet> bi = bullets.iterator(); bi.hasNext(); ) {
                ...
                for (Iterator<Asteroid> ai = asteroids.iterator(); ai.hasNext(); ) {
                    Asteroid a = ai.next();
    ...
    public void draw(Canvas canvas) {
        ...
        if (bullets != null) {
            ...
            for (Bullet b : bullets) b.draw(canvas, p);
        }

        if (asteroids != null) {
            p.setColor(Color.rgb(255, 112, 67));
            for (Asteroid a : asteroids) a.draw(canvas, p);
        }
